...
/**
 * Class of implementation command "open". Extends class "CommandImplementation".
 *
 * @author Siarhei Tuzhyk
 * @version 1.0
 * @since 20.11.2016
 */
public class OpenCommand extends CommandImplementation {
    /**
    ...
    public boolean canImplementation(Command command) {
        return command.getCommand().equals(CommandsConstant.OPEN);
    }
    ...
    public CommandResult implementationOfCommand(WebDriver webDriver, Command command) {
        ...
        BigDecimal expectedTime = new BigDecimal(command.getExpected());
        boolean result = expectedTime.compareTo(runTime) == 1;
