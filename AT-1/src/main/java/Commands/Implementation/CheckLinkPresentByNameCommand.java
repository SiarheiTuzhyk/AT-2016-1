...
/**
 * Class or implementation command "checkLinkPresentByNameCommand". Extends class "CommandImplementation".
 *
 * @author Siarhei Tuzhyk
 * @version 1.0
 * @since 20.11.2016
 */
public class CheckLinkPresentByNameCommand extends CommandImplementation {
    /**
    ...
    public boolean canImplementation(Command command) {
        return command.getCommand().equals(CommandsConstant.CHECK_LINK_PRESENT_BY_NAME);
    }
    ...
    public CommandResult implementationOfCommand(WebDriver webDriver, Command command) {
        ...
        if (!result) {
            try {
                webDriver.findElement(By.linkText(command.getExpected()));
            } catch (Exception ex) {
                result = false;
            }
        }
     ...
     */
    private boolean getResultByNullURL(WebDriver webDriver) {
