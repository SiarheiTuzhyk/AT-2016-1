...
/**
 * Class of implementation command "checkPageTitle". Extends class "CommandImplementation".
 *
 * @author Siarhei Tuzhyk
 * @version 1.0
 * @since 20.11.2016
 */
public class CheckPageTitleCommand extends CommandImplementation {
    /**
    ...
    public boolean canImplementation(Command command) {
        return command.getCommand().equals(CommandsConstant.CHECK_PAGE_TITLE);
    }
    ...
    public CommandResult implementationOfCommand(WebDriver webDriver, Command command) {
        ...
        if (!result) {
            result = webDriver.getTitle().equals(command.getExpected());
        }
